...
class _BillScreenState extends State<BillScreen> {
  ...
  Future<void> _generateBill() async {
    ...
    final cnicDigits =
        _cnicController.text.replaceAll(RegExp(r'[^0-9]'), '');
    if (cnicDigits.length != 16) {
      _showMessage('CNIC must contain exactly 16 digits.');
      return;
    }
    ...
    final contactDigits =
        _contactController.text.replaceAll(RegExp(r'[^0-9]'), '');
    if (contactDigits.length != 11 || !contactDigits.startsWith('03')) {
      _showMessage('Contact number must be 11 digits and start with 03.');
      return;
    }
    ...
    try {
      ...
      await firestore.runTransaction((transaction) async {
        ...
        for (final item in _rentedItems) {
          final ref =
              firestore.collection('inventory').doc(item.itemId);
                ...
                children: [
                  ...
                  Text(
                    ...
                    style: TextStyle(
                      ...
                      fontWeight: FontWeight.w700,
                      color:
                          date == null ? _mutedText : _darkBrown,
                    ),
                  ),
                ],
              ),
      ...
      body: SafeArea(
        child: LayoutBuilder(
          builder: (_, constraints) {
            ...
            final horizontal = mobile ? 14.0 : tablet ? 24.0 : 34.0;
            ...
            return SingleChildScrollView(
              ...
              child: Center(
                child: ConstrainedBox(
                  ...
                  child: Form(
                    key: _formKey,
                    child: _buildFormContent(mobile),
                  ),
                ),
              ),
            );
